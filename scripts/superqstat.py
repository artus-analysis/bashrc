...
"""
    Advanced qstat. This script provides information on jobs on the local batch 
    system for all users.

    Forked from Artus:
    https://github.com/artus-analysis/Artus/blob/master/Utility/scripts/qstat-tools.py
"""
...
# These are needed for get_tty_size:
import array
import fcntl
import termios
...
def print_qstat_table(args, previous_users=[]):
	...
	for user, symbol in zip(users, symbols):
		...
		line += progress_bar(len(parsed_qstat_output[user].get("r", [])),
		                     ...
		                     jobs, tty_width-len(line)-2)
		qstat_table += "\n" + symbol + " "
		if user == getpass.getuser():
			qstat_table += "\033[0;31;47m" + line + "\033[0m"
		else:
			qstat_table += line
...
def main():
	...
	parser.add_argument("-c", "--continuous-mode", default=0, type=int,
	                    help="Continuous mode. Argument is refresh interval. Default is non-continuous mode (0).")
